@dispCentr
Feature: smoke test #1, Просмотр карточки заявки роль СТП
  ...
  Scenario: authorization at STP
    ...
    ##просмотр  обычной бесплатной заявки
  Scenario: check View form of Normal Free Request in status "New" on all elements
    ...
    And check that element with name "Статус выполнения БП" is displayed in request
   ...
   # And check that element with name "Житель просит связаться по вопросу" is not displayed in request
    ...
    And check that button with text "Перезапустить БП" is visible in request

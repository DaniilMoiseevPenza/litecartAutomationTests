@dispCentr
Feature: smoke test #1, Проверка поиска по номеру заявки роль СТП
  ...
  Scenario: check the search on number request
    ...
    And type to input with name searchOnNumberRequest text: "0100-0101-007135/18"
    And click button findRequest
    And click on first request
    And check that element with name "0100-0101-007135/18" is displayed in request
  ...
  Scenario: check the search on number request after filtr
    ...
    And click fieldSearchOnNumberRequest in startPage
    And type to input with name searchOnNumberRequest text: "0100-0101-007085/18"
    And click button findRequest
    And click on first request
    And check that element with name "0100-0101-007085/18" is displayed in request

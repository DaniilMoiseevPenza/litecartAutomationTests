...
Feature: smoke test #1, Проверка страницы Фильтр отключений
  ...
  Scenario: check filter region
    ...
    And type to input text "Центральный административный округ" in filtr "Округ" in filtrOff
    And click first string with name "Центральный административный округ"
#    And click first region in fieldRegion in filtrOff
#    And check fieldOds is disabled
    And click to fieldDistrict in filtrOff
    And type to input text "муниципальный округ Арбат" in filtrDistrictOff

@dispCentr
Feature: smoke test #1, Проверка стартовой страницы Диспетчерского центра роль РДС
  ...
  Scenario: check Dispatch Center page on all elements
    ...
    And check that element with name "Диспетчер РДС" is displayed in top
    And check that element with name "Тестовый РДС Авто" is displayed in top

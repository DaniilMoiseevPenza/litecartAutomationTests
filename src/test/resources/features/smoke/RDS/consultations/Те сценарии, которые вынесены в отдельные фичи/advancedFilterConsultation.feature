@consultation @rds
Feature: smoke test #1, Проверка фильтрации консультации под РДС
...
#    And check that element with name "Оказал консультацию:" is displayed // временно убрали
  ...
  Scenario: check filter address
    ...
    And press button with text "Сбросить фильтр"
    And click checkbox of problem consultation in filtr
    And type to input text "улица Новый Арбат, дом 25" in filtr "Адрес" in filtrConsultation
    And click first address with text "улица Новый Арбат, дом 25"
    And press button with text "Применить"
    And click first consultation
    And check that field "Адрес" is "улица Новый Арбат, дом 25"
    And click collapse consultation
...
#    And click collapse consultation
  ...
  Scenario: check filtr number of request
    ...
    And press button with text "Сбросить фильтр"
    And click checkbox of problem consultation in filtr
    And type to input text "0100-0101-007035/18" in filtr "Номер заявки" in filtrConsultation
    And press button with text "Применить"
    And click first consultation
    And check that field "По заявке" is "0100-0101-007035/18"
    And click collapse consultation
  ...
  Scenario: check filtr number of request, if number of request is not full
    ...
    And press button with text "Сбросить фильтр"
    And click checkbox of problem consultation in filtr
    And type to input text "0300-0308-001258" in filtr "Номер заявки" in filtrConsultation
    And press button with text "Применить"
  ...
  Scenario: check filtr number of request after filtr
    ...
    And press button with text "Сбросить фильтр"
    And click checkbox of problem consultation in filtr
    And type to input text "1-й Смоленский переулок, дом 13" in filtr "Адрес" in filtrConsultation
    And click first address with text "1-й Смоленский переулок, дом 13"
    And type to input text "0100-0101-007035/18" in filtr "Номер заявки" in filtrConsultation
    And press button with text "Применить"
    And click first consultation
    And check that field "По заявке" is "0100-0101-007035/18"
    And check that field "Адрес" is not "1-й Смоленский переулок, дом 13"
    And click collapse consultation
  ...
  Scenario: check filtr number of consultation after filtr
    ...
    And click first consultation
    And check that field "По заявке" is "0100-0101-007035/18"
    And check that field "Адрес" is not "1-й Николощеповский переулок, дом 20/4"
    And click collapse consultation
    And type to input with name searchOnNumberConsultation text: "К0100-0101-001909/18"
    And click button findRequest
    And check that number request in grid is "К0100-0101-001909/18"
    And click first consultation
    And check that field "По заявке" is not "0100-0101-007035/18"
    And check that field "Адрес" is not "1-й Николощеповский переулок, дом 20/4"
    And click collapse consultation

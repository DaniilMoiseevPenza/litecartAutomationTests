@dispCentr
Feature: smoke test #1, Проверка стартовой страницы Диспетчерского центра роль ОДС
  ...
  Scenario: check Dispatch Center page on all elements
    ...
    And check that element with name "Диспетчер OДС" is displayed in top
    And check that element with name "Тестовая ОДС Авто" is displayed in top

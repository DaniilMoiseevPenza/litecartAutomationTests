Feature: smoke test #1, Проверка колонок отображения
  ...
  Scenario: check the display of columns in the grid "Номер заявки"
    Given click choose colums
    And click button deselectAll
    And click choose column "Номер заявки"
    And click choose colums apply
    And check that element with name "Номер заявки" is displayed in grid
  ...
  #Scenario: check the display of columns in the grid "Специализация исполнителя"
   # Given click choose colums
    #And click button deselectAll
    #And click choose column "Специализация исполнителя"
    #And click choose colums apply
    #And check that element with name "Специализация исполнителя" is displayed in grid

Feature: smoke test #1, Проверка поиска по номеру заявки
  ...
  Scenario: check the search on number request
    ...
    And type to input with name searchOnNumberRequest text: "1100-0000-244693/18"
    And click button findRequest
    And click on first request
    And check that element with name "1100-0000-244693/18" is displayed in request

...
#    And click collapse
  ...
  Scenario: Проверка поиска отключений по фильтру.Поле № ОДС
    And click button filtroff
    And click button with text "Сбросить фильтр" in filtrOff
    And type to input text "Диспетчерская РДС (Арбат)" in filtr "№ ОДС"
    And click to first ods in filtrOdsOff
    And click button with text "Применить" in filtrOff
    And click first off

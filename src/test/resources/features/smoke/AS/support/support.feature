...
Feature: smoke test #1, Проверка формы обратной связи в меню поддержка
  ...
  Scenario: test form settings
    ...
    And check that element with name "При поступлении заявок" is displayed in form settings
    And check that element with name "При поступлении комментариев по заявкам" is displayed in form settings

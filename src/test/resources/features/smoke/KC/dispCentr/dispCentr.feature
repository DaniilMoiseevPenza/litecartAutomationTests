@dispCentr
Feature: smoke test #1, Проверка стартовой страницы Диспетчерского центра роль КЦ
  ...
  Scenario: check Dispatch Center page on all elements
    ...
    And check that element with name "Оператор КЦ" is displayed
    #And check that element with name "Оператор КЦ" is displayed
    And check that element with name "КЦ" is displayed
    And check that element with name "Заявки" is displayed
    And check that element with name "Новые" is displayed
    And check that element with name "Прозвонить отклоненные" is displayed

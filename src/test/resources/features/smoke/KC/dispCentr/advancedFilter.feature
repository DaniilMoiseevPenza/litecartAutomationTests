@dispCentr
Feature: smoke test #1, Проверка фильтрации заявок роль КЦ

  Background:  authorization at KC
...
#    And type to input "Новый арбат, дом 25" in filtr "Адрес" in dispCentr
...
#    And check that enterances in grid is "1"
  ...
  Scenario Outline: check filtr date of create
    And click to field date create with
    And type to input text "<Create date with>" in "<Filter name>" "с" in filtr
    And click empty in form filtr of request
    And click to field date create by
    And type to input text "<Create date by>" in "<Filter name>" "по" in filtr
    And click empty in form filtr of request
    And press button with text "Применить"
    And click choose colums
    And click button deselectAll
    ...
    And click choose colums apply
    And check that date create of request in grid between "<Create date with>" and "<Create date by>"
  Examples:
  |Create date with|Create date by  |Filter name  |
  |01.12.2018 10:00|23.12.2018 10:00|Дата создания|
#
#

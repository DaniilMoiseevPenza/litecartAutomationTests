...
#    Then click filter
#    Then press button with text "Сбросить фильтр"
#    Then press button with text "Расширенный фильтр"
#    Then click to field date create with
#    Then type to input text "01.09.2019 10:00" in "Дата создания" "с" in filtr
#    And click empty in form filtr of request
#    Then click to field date create by
#    Then type to input text "01.11.2019 10:00" in "Дата создания" "по" in filtr
#    And click empty in form filtr of request
#
#  Scenario Outline: check filtr <Название фильтра>
#    And type to input "по" in filtr "<Название фильтра>" in dispCentr
#    And check that first adress in filtr adress is not visible
#    And type to input "<Значение для ввода>" in filtr "<Название фильтра>" in dispCentr
##    And click first address with text "улица Новый Арбат, дом 25"
#    And press button with text "Применить"
#    And click choose colums
#    And click button deselectAll
#    And click choose column "<Название колонки>"
#    And click choose colums apply
##    Then check that "Адрес дома" in grid is "улица Новый Арбат, дом 25"
##    And check that "Квартира" in grid is "1"
##    And check that "Этаж" in grid is "1"
##    And check that "Подъезд" in grid is "1"
#    And check that column in grid is "<Значение фильтра>"
##    And check that column in grid is "1"
##    And check that column in grid is "1"
##    And check that column in grid is "1"
...
#  Examples:
#    |Название фильтра         |Значение для ввода                                |Значение фильтра                                  |Название колонки |
##    |Адрес                    |Арбат 25                                          |улица Новый Арбат, дом 25                         |Адрес дома       |
#    |Подъезд                  |1                                                 |1                                                 |Подъезд          |
#    |Этаж                     |1                                                 |1                                                 |Этаж             |
#    |Квартира                 |1                                                 |1                                                 |Квартира         |
##    |Округ                    |Центральный административный округ                |Центральный административный округ                |ЦАО              |
#    |Контактное лицо          |тест                                              |тест                                              |Контактное лицо  |
#    |Платность                |бесплатная                                        |бесплатная                                        |Платная          |
#    |Приоритет                |Обычная                                           |Обычная                                           |Приоритет        |
#    |Статус                   |Новая                                             |Новая                                             |Статус           |
#    |Причина отмены           |Заявитель обратился в стороннюю организацию       |Заявитель обратился в стороннюю организацию       |Причина отмены   |
#    |Причина отклонения       |Множественное обращение по данной проблеме/дефекту|Множественное обращение по данной проблеме/дефекту|Причина отклонения|
##    |Создал заявку (должность)|                  |||
##    |Создал заявку (ФИО)      |                  |||
##    |Закрыл заявку (ФИО)      |                  |||
##    |Источник                 |                  |||
#
...
#    And click choose colums apply
#    And check that column in grid is "ЦАО"
#   # And check that column "Округ" is "ЗАО"
##
##

@dispCentr
Feature: smoke test, Проверка стартовой страницы Диспетчерского центра на все элементы, роль ЦДС
  ...
  Scenario: check Dispatch Center page on all elements
    ...
    And check that element with name "Диспетчер ЦДС" is displayed in top
    And check that element with name "Тестовая ЦДС Авто" is displayed in top
...
#    And check that element with name "Закрытие заявок" is displayed
    ...
    And scroll to column with name "Вид выполненных работ"
    ...
    And check that button with text "Выписка" is not displayed
